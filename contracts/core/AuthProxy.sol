...
import "../interfaces/IUpgradeableMaster.sol";
...
/// @author Kontos
contract AuthProxy is IUpgradeable {
    ...
    function upgradeTarget(address newTarget, bytes calldata newTargetUpgradeParameters) external override {
        // TODO check with cross-chain contract
        (bool passAuth,) = getTarget().delegatecall(abi.encodeWithSignature("auth(bytes)", newTargetUpgradeParameters));
        require(passAuth, "ii");
